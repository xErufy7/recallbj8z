...
export const applyAiEffect = (s: GameState, effect: SerializableEffect): Partial<GameState> => {
    ...
    if (effect.mindset) updates.general!.mindset = Math.min(100, Math.max(0, s.general.mindset + effect.mindset));
    if (effect.health) updates.general!.health = Math.min(100, Math.max(0, s.general.health + effect.health));
    if (effect.money) updates.general!.money = s.general.money + effect.money; // Money can be negative
    if (effect.efficiency) updates.general!.efficiency = Math.min(100, Math.max(1, s.general.efficiency + effect.efficiency));
    if (effect.romance) updates.general!.romance = Math.min(100, Math.max(0, s.general.romance + effect.romance));
    if (effect.experience) updates.general!.experience = Math.min(999, Math.max(0, s.general.experience + effect.experience));
    if (effect.luck) updates.general!.luck = Math.min(100, Math.max(0, s.general.luck + effect.luck));
